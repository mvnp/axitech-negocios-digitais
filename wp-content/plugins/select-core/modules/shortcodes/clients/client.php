...
class Client implements ShortcodeInterface{
	...
	public function vcMap() {
		if(function_exists('vc_map')){
			vc_map( 
				array(
					'name' => esc_html__('Client', 'select-core'),
					'base' => $this->base,
	...
	public function render($atts, $content = null) {
		...
		if(is_numeric($params['image'])) {
			...
			$params['image_alt'] = esc_attr(get_post_meta(attachment_url_to_postid($params['image']), '_wp_attachment_image_alt', true));
		}
		if(is_numeric($params['hover_image'])) {
			$params['hover_image'] = wp_get_attachment_url($params['hover_image']);
			$params['hover_image_alt'] = esc_attr(get_post_meta(attachment_url_to_postid($params['hover_image']), '_wp_attachment_image_alt', true));
		}
		...
		$html = qode_core_get_independent_shortcode_module_template_part('templates/client-template', 'clients', '', $params);
