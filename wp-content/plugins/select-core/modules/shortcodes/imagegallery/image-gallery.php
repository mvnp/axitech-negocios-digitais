...
class ImageGallery implements ShortcodeInterface{
	...
	public function render($atts, $content = null) {
		...
		$html = qode_core_get_independent_shortcode_module_template_part('templates/' . $template, 'imagegallery', '', $params);
	...
	private function getGalleryImages($params) {
		...
		if ($params['images'] !== '') {
			...
			foreach ($image_ids as $id) {
				...
				$image['url'] = isset($img[0]) ? $img[0] : '';
				$image['width'] = isset($img[1]) ? $img[1] : '';
				$image['height'] = isset($img[2]) ? $img[2] : '';
