...
if(!function_exists( 'startit_qode_add_custom_nav_fields' )) {
	function startit_qode_add_custom_nav_fields( $menu_item ) {
	...
	add_filter( 'wp_setup_nav_menu_item', 'startit_qode_add_custom_nav_fields');
}

if(!function_exists( 'startit_qode_update_custom_nav_fields' )) {
	/**
	 ...
	 * @since       1.0
	 */
	function startit_qode_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
	...
	// save menu custom fields
	add_action( 'wp_update_nav_menu_item', 'startit_qode_update_custom_nav_fields', 10, 3 );
}


if(!function_exists( 'startit_qode_edit_walker' )) {
	/**
	 ...
	 * @since       1.0
	 */
	function startit_qode_edit_walker( $walker, $menu_id ) {

		return 'QodeStartit_Walker_Nav_Menu_Edit_Custom';

	}
	// edit menu walker
	add_filter( 'wp_edit_nav_menu_walker', 'startit_qode_edit_walker', 10, 2 );
}
